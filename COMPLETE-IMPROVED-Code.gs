// ==================== PI EVENTS APP - COMPLETE IMPROVED CODE.GS ====================
// Copy this ENTIRE file into your Code.gs in Apps Script
// FIXED: Preserves existing events, statuses, and manual edits

// ==================== CONFIGURATION ====================
const SOURCE_SPREADSHEET_ID = '1NiiWMcEEwjiU_DeVuUre_Qxyf5DGqEwG8Z8mYIRMuGU';
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('🔄 Import New Approved Events', 'runFullAutomationUI')
    .addSeparator()
...
// ==================== MAIN AUTOMATION - IMPORT NEW EVENTS ONLY ====================
function runFullAutomationUI() {
  const ui = SpreadsheetApp.getUi();
  const response = ui.alert(
    '🔄 Import New Approved Events',
    'This will:\n\n' +
    '1. Import NEW approved events from PI Work Flow\n' +
    '2. Skip events already in CURATED (preserves status & edits)\n' +
    '3. Find ticket links automatically\n' +
    '4. Detect categories\n' +
    '5. Add booking guide information\n\n' +
    'Continue?',
    ui.ButtonSet.YES_NO
  );
  ...
  try {
    const result = importApprovedEvents();
    addCuratedDropdowns();
    removeDuplicates();
    ...
    ui.alert(
      ...
      'Check the CURATED tab to review new events.',
      ui.ButtonSet.OK
    );
  } catch (error) {
...
function importApprovedEvents() {
  ...
  monthlyTabs.forEach(tabName => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      newEvents.push([
        ...
        imageUrl,
        ticketLink,
        'Pending', // New events start as Pending
  ...
  // Append new events (preserves existing events)
  curatedSheet.getRange(curatedSheet.getLastRow() + 1, 1, newEvents.length, 13)
    .setValues(newEvents);
...
function refreshBookingGuides() {
  ...
  if (!hasGuideCol || !guideUrlCol || !bookingNoteCol) {
    throw new Error('Booking guide columns not found. Run "Import New Approved Events" first.');
  }
