...
/**
 * Find venue details (address, postcode, maps URL) for an event.
 * Uses same matching logic as access features.
 */
function findVenueDetails(event) {
    ...
    function matchDetails(name) {
        ...
        for (const [canonical, details] of Object.entries(VENUE_DETAILS)) {
            if (n.includes(canonical) || canonical.includes(n)) return details;
        }
        return null;
    }

    if (event['VENUE']) {
        const result = matchDetails(event['VENUE']);
        if (result) return result;
    }
    if (event['EVENT']) {
        const result = matchDetails(event['EVENT']);
        if (result) return result;
    }
    return null;
}
